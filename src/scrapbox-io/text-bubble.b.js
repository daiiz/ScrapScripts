...
exports.$getTextBubble = function () {
  var $textBubble = $(
    `<div class="daiiz-text-bubble related-page-list daiiz-card daiiz-card-root"></div>`
  );
  return $textBubble;
};
...
var parse = function (fullStr, startIdx, depth, seekEnd) {
  ...
  while (startIdx < l) {
    ...
    if (subStr.startsWith(DOUBLE_BRACKET_OPEN) && !openInlineCode) {
      ...
      var str =
        BRACKET_OPEN + fullStr.substring(token[0], token[1]) + BRACKET_CLOSE;
      ...
      var str =
        INLINE_CODE + fullStr.substring(token[0], token[1]) + INLINE_CODE;
...
var previewPageText = function ($root, $bubble, title, rowHash) {
  ...
  // TODO: src/gyazo-com/text-bubble.js 機能でCORS突破できない
  $.ajax({
    type: "GET",
    ...
    if (contents.length > 0) {
      $bubble.html(
        `<div class="daiiz-bubble-text ${extraClassName}">${contents.join(
          "<br>"
        )}</div>`
      );
      $bubble.show();
